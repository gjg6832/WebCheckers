...
public class WebServer {
  ...
  public static final String MOVE_URL = "/validateMove";
   ...
   */
  public WebServer(GameCenter gameCenter, final TemplateEngine templateEngine, final Gson gson) {
    // validation
  ...
  public void initialize() {
    ...
    post(CONNECT, new PostGameRoute(gameCenter, templateEngine));

    post(CHECK_TURN_URL, new PostCheckTurnRoute(gson, gameCenter));

    post(MOVE_URL, new PostMoveRoute(gameCenter));

    post(SUBMIT_URL, new PostSubmitTurnRoute(gameCenter, gson));

    get(SIGN_OUT_URL, new GetSignoutRoute(gameCenter, gson));

    post(RESIGN_URL, new PostResignRoute(gameCenter, gson));

    post(BACKUP_MOVE, new PostBackUpRoute(gameCenter,gson) );
